...
# Adaptive Trainings-App mit Streamlit + Intervals.icu API + API-Key-Testformular (Basic Auth, Session & Debug)
...
if st.sidebar.button("Test starten") and api_key_test:
    response = session.get("https://intervals.icu/api/v1/athlete", auth=("API_KEY", api_key_test))
...
if st.button("📊 Plan generieren") and api_key:
    ...
    r = session.get("https://intervals.icu/api/v1/athlete", auth=("API_KEY", api_key))
