...
# Adaptive Trainings-App mit Streamlit + Intervals.icu API + API-Key-Testformular
...
if st.sidebar.button("Test starten") and api_key_test:
    headers_test = {"Authorization": f"Bearer {api_key_test}"}
    response = requests.get("https://intervals.icu/api/v1/athlete", headers=headers_test)
...
if st.button("📊 Plan generieren") and api_key:
    ...
    r = requests.get("https://intervals.icu/api/v1/athlete", headers=headers)
